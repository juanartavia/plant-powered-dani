...
function getPilatesCalendarId() {
    const id = PropertiesService.getScriptProperties().getProperty("PILATES_CALENDAR_ID");
    if (!id) {
        throw new Error("PILATES_CALENDAR_ID no configurado en Script Properties. En testing, ejecutar " +
...
function setupPilatesTestCalendar() {
...
function doGet(e) {
    ...
    const type = (_b = (_a = e === null || e === void 0 ? void 0 : e.parameter) === null || _a === void 0 ? void 0 : _a.type) !== null && _b !== void 0 ? _b : "";
...
function fetchAvailability(type) {
    ...
    const nearestTimeslot = new Date(Math.floor(new Date().getTime() / durationMs) * durationMs);
    ...
    const end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + DAYS_IN_ADVANCE));
    const response = Calendar.Freebusy.query({
        timeMin: now.toISOString(),
        timeMax: end.toISOString(),
        items: CALENDARS.map((id) => ({ id })),
    });
    const events = CALENDARS.map((calendarId) => {
        const busyTimes = response.calendars[calendarId].busy;
    ...
    const timeslots = [];
    for (let t = nearestTimeslot.getTime(); t + durationMs <= end.getTime(); t += durationMs) {
        const start = new Date(t);
        const end = new Date(t + durationMs);
        if (start.getTime() <= minBookingTime.getTime())
            continue;
        const startTZ = new Date(Utilities.formatDate(start, TIME_ZONE, "yyyy-MM-dd'T'HH:mm:ss"));
        if (startTZ.getHours() < WORKHOURS.start)
            continue;
        if (startTZ.getHours() >= WORKHOURS.end)
            continue;
        if (WORKDAYS.indexOf(startTZ.getDay()) < 0)
            continue;
        if (type === "pilates") {
            if (startTZ.getDay() !== PILATES_DAY_OF_WEEK)
                continue;
            if (startTZ.getHours() !== PILATES_START_HOUR || startTZ.getMinutes() !== 0)
                continue;
            ...
            if (inscritos >= MAX_PILATES_PARTICIPANTS)
                continue;
        }
        else if (events.some((event) => event.start < end && event.end > start)) {
            continue;
        }
...
function addCancelacionesColumnsToClientes() {
...
function addServicioColumnsToClientes() {
    const sheet = getSheet("Clientes");
...
function cleanupCorruptedClientesSheet() {
    const sheet = getSheet("Clientes");
    ...
    while (contiguousEnd < allValues.length &&
    ...
    if (lostRows.length === 0) {
        Logger.log(`cleanupCorruptedClientesSheet: no se encontraron filas "perdidas" más allá de la ` +
    ...
    lostRows.forEach((lost) => {
        ...
        const fechaNacimientoNormalizada = fechaNacimientoRaw instanceof Date
        ...
        movedSummary.push(`Fila ${lost.rowNumber} -> Fila ${nextFreeRow} (correo: ${lost.values[0]}, ` +
...
function fixFechaNacimientoFormatInClientes() {
    const sheet = getSheet("Clientes");
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) {
    ...
    for (let i = 0; i < numDataRows; i++) {
        ...
        if (correo === "")
            continue; // fila vacía dentro del rango — no debería pasar, se ignora
        const rawValue = fechaNacimientoValues[i][0];
        if (!(rawValue instanceof Date))
            continue; // ya es texto plano, idempotente
        ...
        Logger.log(`fixFechaNacimientoFormatInClientes: fila ${rowNumber} (correo: ${correo}) — ` +
...
function correctOffByOneDayBirthdates() {
    const scriptProperties = PropertiesService.getScriptProperties();
    if (scriptProperties.getProperty(BIRTHDATE_OFFBYONE_FIX_FLAG) === "true") {
        Logger.log("correctOffByOneDayBirthdates: YA SE EJECUTÓ ANTES (marca guardada en Script " +
    ...
    for (let i = 0; i < numDataRows; i++) {
        ...
        if (!isoDatePattern.test(before)) {
            Logger.log(`correctOffByOneDayBirthdates: fila ${rowNumber} (correo: ${correo}) — valor ` +
        ...
        cell.setValue(after);
        Logger.log(`correctOffByOneDayBirthdates: fila ${rowNumber} (correo: ${correo}) — ` +
            `antes: "${before}" -> después: "${after}"`);
        fixedCount++;
    }
    scriptProperties.setProperty(BIRTHDATE_OFFBYONE_FIX_FLAG, "true");
    Logger.log(`correctOffByOneDayBirthdates: ${fixedCount} fila(s) corregida(s) de ${numDataRows} ` +
...
function migrateCedulaToTipoNumeroId() {
...
function addEventIdColumnToNutricion() {
...
function addEventIdColumnToCuposPilates() {
...
function normalizeSheetDateCell(value, pattern) {
...
const VALID_TIPO_ID_VALUES = ["cedula", "pasaporte", "licencia", "otro"];
...
function assertValidTipoId(tipoId) {
    if (VALID_TIPO_ID_VALUES.indexOf(tipoId) < 0) {
...
function assertMinimumAge(birthdateStr) {
...
function findClientByEmail(correo) {
    const sheet = getSheet("Clientes");
...
function getClientPaymentStatus(correo) {
    const client = findClientByEmail(correo);
    if (!client)
        return { cancelaciones_tardias: 0, requiere_pago: false };
    return {
...
function incrementClientLateCancellation(correo) {
...
function resetClientLateCancellationCounter(correo) {
    const lock = LockService.getScriptLock();
    lock.waitLock(10000);
    try {
        const sheet = getSheet("Clientes");
...
function notifyLateCancellation(correo, token, accion) {
...
function upsertClient(data, type) {
...
function ensureCuposPilatesPlainTextFormat(sheet) {
...
function appendBookingToSheet(type, data) {
...
function markBookingRowError(type, token) {
...
function updateNutricionCalendarInfo(token, eventId, meetLink) {
    const sheet = getSheet("Nutrición");
    const row = findRowByToken(sheet, token);
    if (row < 0)
        return;
    ...
    if (meetLink)
        sheet.getRange(row, NUTRICION_MEET_LINK_COL).setValue(meetLink);
}
...
function rollbackPilatesCupo(fecha, hora) {
    const lock = LockService.getScriptLock();
    lock.waitLock(10000);
    try {
        const cuposSheet = getSheet("Cupos_Pilates");
...
function createCalendarEventWithMeet(calendarId, summary, startTime, endTime, description, guestEmail, wantsMeet) {
    const eventResource = {
        summary,
        description,
...
function bookNutricionCalendarEvent(type, token, startTime, endTime, nombre, apellido, email, phone, tipoId, numeroId, birthdate, language, modalidad) {
    ...
    try {
        ...
        const hasConflict = CALENDARS.some((calId) => possibleEvents.calendars[calId].busy.length > 0);
        if (hasConflict) {
            throw new Error("SLOT_NO_DISPONIBLE");
        }
        const { eventId, meetLink } = createCalendarEventWithMeet(calendarId, `Appointment with ${nombre} ${apellido}`, startTime, endTime, description, email, wantsMeet);
...
function bookPilatesCalendarEvent(startTime, endTime, nombre, apellido, email, phone, tipoId, numeroId, birthdate, language) {
    ...
    try {
        ...
        if (eventId) {
            ...
            const created = createCalendarEventWithMeet(calendarId, "Clase de Pilates", startTime, endTime, description, email, true // pilates siempre es virtual
            );
...
function bookTimeslot(type, timeslot, nombre, apellido, email, phone, tipoId, numeroId, birthdate, language, modalidad, clientTimezone) {
...
function findBookingByToken(token) {
    const sheetNames = ["Nutrición", "Pilates"];
...
function parseSheetDateTime(fecha, hora) {
...
function cancelNutricionCalendarEvent(token) {
    const sheet = getSheet("Nutrición");
    const row = findRowByToken(sheet, token);
    if (row < 0)
        return;
    ...
    try {
        ...
        Logger.log(`cancelNutricionCalendarEvent: error eliminando evento ${eventId}: ${e.message}`);
    }
}
...
function leavePilatesSlot(fecha, hora, email) {
    const calendarId = getPilatesCalendarId();
    const lock = LockService.getScriptLock();
    lock.waitLock(10000);
    try {
        ...
        if (eventId) {
            if (nuevoInscritos <= 0) {
                try {
                    ...
                    Logger.log(`leavePilatesSlot: error eliminando evento ${eventId}: ${e.message}`);
                }
                ...
                try {
                    const existingEvent = Calendar.Events.get(calendarId, eventId);
                    const attendees = (existingEvent.attendees || []).filter((a) => (a.email || "").trim().toLowerCase() !== email.trim().toLowerCase());
                    // sendUpdates: 'none' — decisión del 18 jul: el cliente NO debe recibir la invitación
                    // nativa de Google Calendar (expone cédula/fecha de nacimiento de la descripción del
                    // evento sin ningún diseño). El único correo que debe recibir el cliente al agendar es
                    // el de confirmación propio (US-12). Para volver a activar la invitación nativa de
                    ...
                    Logger.log(`leavePilatesSlot: error removiendo invitado del evento ${eventId}: ${e.message}`);
                }
            }
        }
        SpreadsheetApp.flush();
...
function joinPilatesSlot(fecha, hora, nombre, apellido, email, phone, tipoId, numeroId, birthdate, language) {
    const calendarId = getPilatesCalendarId();
    const description = `Name: ${nombre} ${apellido}\nEmail: ${email}\nPhone: ${phone}\nID: ${tipoId} ${numeroId}\nDate of birth: ${birthdate}\nLanguage: ${language}\nAppointment type: pilates\nModality: virtual`;
    const lock = LockService.getScriptLock();
    lock.waitLock(10000);
...
function rescheduleBooking(token, newTimeslot, clientTimezone) {
    const booking = findBookingByToken(token);
    if (booking.estado === "Cancelada") {
    ...
    if (booking.sheetName === "Pilates") {
        ...
        joinPilatesSlot(newFecha, newHora, booking.nombre, booking.apellido, booking.correo, booking.telefono, booking.tipoId, booking.numeroId, booking.birthdate, booking.language);
        leavePilatesSlot(booking.fecha, booking.hora, booking.correo);
        ...
        try {
            ...
            if (eventId) {
                ...
                }, CALENDARS[0], eventId, { sendUpdates: "none" });
            }
            else {
...
const CONSULTORIO_DIRECCION = "Santa Ana Town Center<br>Work Space Republic – Segundo piso<br>Consultorio #33";
...
const TEMPLATE_FILE_BY_TIPO_IDIOMA = {
    nutricion: {
...
function formatFechaDisplay(instant, idioma, zona = TIME_ZONE) {
...
function testSendConfirmationEmails() {
