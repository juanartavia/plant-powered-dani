...
> Última actualización: 18 julio 2026 — **Invitación nativa de Google Calendar suprimida** (`sendUpdates: 'none'`) en creación/actualización de eventos — el cliente ya no debe recibir el correo nativo de Calendar, que exponía cédula/fecha de nacimiento en texto plano. Se agregó el scope `userinfo.email` al manifest (faltaba para `Session.getActiveUser()` en `testSendConfirmationEmails()`). US-11 (correo de confirmación) sigue en progreso, render verificado, envío real pendiente de confirmar — **antes de correr `testSendConfirmationEmails()` desde el editor hay que aceptar el nuevo permiso que Google va a pedir**. Deploy activo: v26.
