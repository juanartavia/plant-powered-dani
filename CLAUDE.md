...
> Última actualización: 28 julio 2026 — **Frontend de US-31 (página de gestión de citas: menú, confirmar asistencia, reagendar, cancelar) y brandbook de US-28 recuperados y desplegados** — no estaban perdidos solo en el sentido de "sin commitear desde el 21 de julio" como se creía: el archivo `manage-booking.tsx`, sus 4 hooks, el logo y las fuentes Jost habían desaparecido del disco por completo (ni siquiera estaban en la copia corrupta de OneDrive) y se recuperaron vía el historial de versiones de OneDrive.com. De paso se resolvió **US-41** (quitar el título `CardTitle` redundante en la pantalla de cancelación). Ver nota técnica #36 para el detalle del episodio de recuperación completo (backend + frontend). Verificado con Playwright (4 pantallas, desktop+375px) y test-harness backend 148/148. Deploy activo: **v48**. **Nota:** las secciones 0/11/12 de este documento ya reflejan la recuperación de US-31/US-28; el detalle fino del historial de deploys entre v28 y v45 para el resto de tarjetas (US-13/US-30, US-32, primera versión de US-33) sigue pendiente de reconstruir (ver Sprint 3, sección 11) — no bloquea nada, pero léase con esa salvedad.
...
3. **Todo lo siguiente ya está Done y validado en testing real:** US-19 (reorden de pantallas), US-18 (ID flexible), la modificación cliente_nutricion/cliente_pilates, US-16 (mes actual por defecto), 3 ajustes sin número de US del día del demo (quitar dark mode, quitar footer del repo base, restringir pilates a sábados 10am), los 4 pedidos de Dani post-demo (US-29 con edad mínima 15 años, idioma solo en Paso 1, banderas SVG reales, pilates con 12hrs de anticipación), **US-31 (frontend de la página de gestión de citas: menú/confirmar/reagendar/cancelar), US-28 (brandbook: paleta de colores, tipografía Jost, logo) y US-41 (quitar título redundante en la pantalla de cancelación)** — estos últimos tres, recuperados y desplegados el 28 de julio tras el incidente de OneDrive (ver nota técnica #36).
...
6. El resto del Sprint 2 según el tablero real de Trello (ver sección 11): US-11, US-12, US-13, US-14, US-20 — todas en Backlog o en progreso parcial. **US-28 ya no está en esta lista — Done, ver punto 3.**
7. **Antes de tocar US-11/US-12**, el usuario tiene carpetas descargadas de Drive (branding/colores, comunicaciones/plantillas de correo, gráficos de Dani) que subirá al repo en una carpeta `design-reference/` separada de `backend/`/`frontend/` cuando lleguemos a esas tarjetas — no asumir que ya están ahí sin confirmar.
...
### Alerta interna de cancelación tardía (US-33, RF-2.5) ✅ CONFIRMADO Y VALIDADO (27 jul, deploy v46)
Cuando un cliente cancela con menos de `CANCELLATION_HOURS` (24hrs) de anticipación, además de la notificación interna general de "cita cancelada" (US-13/US-30) se envía una segunda alerta aparte (`notifyLateCancellation` → `sendNotificacionCancelacionTardia`), dirigida a:
- **Nutrición** → Dani + Ali (Script Properties `DANI_EMAIL` + `ALI_EMAIL`)
- **Pilates** → instructora + Ali (`INSTRUCTORA_EMAIL` + `ALI_EMAIL`)

**Plantilla con branding real (27 jul, reemplaza la primera versión):** la implementación original (sesión no documentada en este archivo — ver nota de reconstrucción pendiente en Sprint 3) armaba el HTML a mano en TypeScript dentro de `renderNotificacionCancelacionTardia()`, con colores propios fuera del brandbook. Funcionaba y ya se había probado en real, pero no se veía consistente con el resto de correos internos del sistema. Se reemplazó por `backend/templates/notificacion_cancelacion_tardia.html`, construida a partir del esqueleto de `notificacion_interna_nueva_cita.html` (mismo logo, misma tipografía Century Gothic/Cambria, misma estructura de tabla y footer) — `renderNotificacionCancelacionTardia()` ahora usa `HtmlService.createTemplateFromFile()`, igual que `renderNotificacionInterna()`. Es **solo español**, igual que las otras dos notificaciones internas (`notificacion_interna_nueva_cita.html`, `notificacion_interna_confirmada.html`) — ninguna de las tres es bilingüe, porque van dirigidas a Dani/instructora/Ali, no al cliente.

**Diferenciación visual exigida por el checklist (punto 4: "visualmente diferenciada de una cancelación normal"):** un badge siempre rojo (`#C0392B`, "⚠️ Cancelación tardía"), una caja de contexto con el mismo rojo mostrando la anticipación real y recordando que el contador de "Clientes" ya se actualizó, filas de "Se canceló el"/"Anticipación real" en rojo dentro de la tabla de detalles, y un botón CTA rojo — ninguno de esos rojos se usa en `notificacion_interna_nueva_cita.html` (esa usa verde/rosado para "agendada", oliva `#B9BD5B` para "reagendada", gris `#8B8B8B` para "cancelada"), así que no se puede confundir con una notificación normal ni a simple vista en la bandeja de entrada. **Aprobado visualmente por el usuario** (capturas de Gmail real recibidas en la cuenta de testing, variantes nutrición y pilates).

**Trade-off de cobertura de tests, aceptado conscientemente:** el mock de `HtmlService.createTemplateFromFile()` en `gas-mock.js` (usado por `backend/test-harness/`) devuelve un HTML fijo que ignora las variables inyectadas — es un punto ciego conocido del harness, documentado desde la nota técnica #39, y es el mismo límite que ya tenían `renderNotificacionInterna()`/`renderConfirmationEmail()`. Antes de este cambio, como el HTML se armaba a mano en TypeScript, el Test 38 SÍ podía buscar texto literal dentro del `htmlBody` (nombre del cliente, fecha/hora, color de alerta, etc.) — esa verificación automática de contenido real del correo ya no es posible. El Test 38 se ajustó para verificar en su lugar: que se usa el archivo de plantilla correcto, el asunto (que se sigue armando en TypeScript, fuera de la plantilla), y los helpers puros que alimentan la plantilla (`formatAnticipacionDisplay`, `TIPO_CITA_LABEL_CANCELACION_TARDIA`) de forma directa. **La verificación de que el HTML final realmente se ve bien con datos reales queda en manos de revisión visual manual** (correr `testSendNotificacionCancelacionTardia()` desde el editor y confirmar en Gmail) — ya se hizo y se aprobó en esta ronda, pero cualquier cambio futuro a esta plantilla necesita el mismo paso manual, el harness no lo va a atrapar.
...
Busca por TOKEN. <24hrs bloquea con `VENTANA_REAGENDAMIENTO_VENCIDA` + incrementa contador de tardías. ≥24hrs valida el nuevo horario igual que `bookTimeslot` (respetando la ventana mínima correspondiente al tipo: 48hrs nutrición, 12hrs pilates). Actualiza Sheet, mueve el evento real de Calendar. Frontend real: ✅ Done — US-31 (`manage-booking.tsx`, pantalla "Reagendar"), recuperado y desplegado 28 jul.

### Flujo cancelación ✅ IMPLEMENTADO Y VALIDADO (US-06)
Busca por TOKEN. SIEMPRE se permite. <24hrs marca tardía + incrementa contador. Actualiza Sheet (nunca borra fila). Elimina el evento real de Calendar. Frontend real: ✅ Done — US-31 (`manage-booking.tsx`, pantalla "Cancelar"; título redundante quitado en US-41), recuperado y desplegado 28 jul.
...
| RF-2.5 | Notificación a Dani/Ali si fuera de ventana | ✅ **Done — US-33, 27 jul (deploy v46)** |
| RF-2.6 | Frontend de reagendar/cancelar | ✅ **Done — US-31, recuperado y desplegado 28 jul (deploy v48)** |
...
| US-28 | Actualizar look & feel según brandbook | 4 | 7/7 | ✅ **Done — recuperado y desplegado 28 jul (deploy v48)**. Implementado en algún punto entre el 21 jul y el incidente de OneDrive; el código se había perdido del disco y se recuperó vía historial de versiones de OneDrive.com. Ver nota técnica #36. |
| US-31 | Frontend de la página de gestión de citas (menú/confirmar asistencia/reagendar/cancelar) | — | — | ✅ **Done — recuperado y desplegado 28 jul (deploy v48)**. Mismo episodio que US-28 — ver nota técnica #36. Backend (US-06) ya estaba validado desde antes. |
| US-41 | Quitar título `CardTitle` redundante ("Cancelar cita") en la pantalla de cancelación de `manage-booking.tsx` | — | — | ✅ **Done (28 jul)** — Deploy v48 |
...
- CLAUDE.md quedó desactualizado entre v27 y v42 — reconstruir el historial de deploys faltante (US-13/US-30, US-32, primera versión de US-33 + fixes) cuando haya oportunidad, no bloquea nada. **US-31 y US-28 ya no forman parte de esta reconstrucción pendiente — se recuperaron y verificaron el 28 jul (deploy v48), ver nota técnica #36.**
...
| Versión actual | **v48** (deployment actualizado 28 jul 2026, US-31 + US-28 + US-41: frontend de gestión de citas y brandbook recuperados) |
| Repo | https://github.com/juanartavia/plant-powered-dani |
...
| Harness de pruebas backend | `backend/test-harness/` — 148 aserciones, todas pasando (28 jul 2026) |
...
| v25 | 18 jul | **Invitación nativa de Google Calendar suprimida** (ajuste puntual, sin número de US): `sendUpdates: "all"` → `"none"` en los 5 `Calendar.Events.insert()`/`.patch()` que crean o actualizan un evento con invitados (`createCalendarEventWithMeet`, alta/baja de invitado en pilates ×3, reprogramar nutrición). El cliente sigue como invitado del evento (no se le saca de `attendees`) — solo se suprime la notificación nativa, que exponía cédula/fecha de nacimiento en texto plano desde la descripción del evento. `Calendar.Events.remove()` (cancelar cita/eliminar evento de pilates) quedó **deliberadamente fuera de este cambio** — el pedido original solo hablaba de insert/patch; revisar si también debe pasar a `'none'` antes de production. Sin dependencia encontrada de Dani/Ali/instructora en la invitación nativa (ven el evento porque son dueños del calendario, no invitados; sus notificaciones internas son un TODO separado de RF-2.3/RF-2.5). 48/48 tests. |
| v26 | 18 jul | **Fix de permiso:** agregado el scope `https://www.googleapis.com/auth/userinfo.email` al manifest (root y `dist/`) — faltaba para que `Session.getActiveUser()` funcione dentro de `testSendConfirmationEmails()` (error real visto al correrla desde el editor: "Specified permissions are not sufficient"). `Session.getActiveUser()` se revisó y se dejó igual — es razonable para una función de testing manual (envía a quien la ejecuta). **La próxima vez que se corra `testSendConfirmationEmails()` desde el editor, Google va a pedir autorizar este permiso nuevo — aceptarlo.** 48/48 tests. |
| v27 | 18 jul | **Fix confirmado en Gmail real (testing):** el bloque de dirección del correo de nutrición presencial mostraba `<br>` como texto literal — `<?= direccion ?>` escapa HTML por default en HtmlService. Cambiado a `<?!= direccion ?>` (imprime sin escapar) en `correo_confirmacion_nutricion_{es,en}.html`. Revisadas las otras variables inyectadas en las 4 plantillas (`tituloConfirmacion`, `nombre_apellido`, `fechaDisplay`, `horaDisplay`, `modalidadDisplay`, `mapsLink`, `wazeLink`, `meetLink`, `linkReagendar`) — ninguna otra trae HTML intencional, así que se dejaron todas escapadas (`<?= ?>`), incluyendo `nombre_apellido` a propósito (viene de input del cliente, nunca debe interpretarse como HTML). 48/48 tests. |
| v28-v45 | 18-27 jul | **Historial no reconstruido todavía** (ver nota en Sprint 3, sección 11) — cubre, entre otros, US-13/US-30 (notificación interna de agendada/reagendada/cancelada), US-31, US-32 (confirmación de asistencia), primera versión de US-33 (alerta de cancelación tardía con HTML armado a mano), y el fix de `backend/package.json`/`copy-to-dist.js` (nota técnica #35). v45 en particular es solo un `clasp version` de este mismo cambio (ver v46) — `clasp deploy` sin `-V` explícito creó automáticamente la versión 46 y esa es la que quedó realmente publicada; v45 existe en el historial de Apps Script pero nunca estuvo detrás del deployment de testing. |
| v46 | 27 jul (noche) | **US-33 — plantilla de branding real para la alerta de cancelación tardía**, aprobada visualmente por el usuario (capturas de Gmail real, nutrición y pilates). `renderNotificacionCancelacionTardia()` pasó de HTML armado a mano en TypeScript a `HtmlService.createTemplateFromFile("notificacion_cancelacion_tardia")`, mismo patrón que `renderNotificacionInterna()`. Detalle completo en sección 3. |
| v47 | 28 jul | `clasp version` manual, quedó huérfana — `clasp deploy` sin `-V` explícito creó automáticamente la v48 (mismo patrón que v45→v46, nota #35/#36). No representa ningún cambio real por sí sola. |
| v48 | 28 jul | **US-31 (frontend de gestión de citas) + US-28 (brandbook) + US-41**, recuperados en la misma sesión — ver nota técnica #36 para el episodio completo. `manage-booking.tsx` + 4 hooks (`useManageBookingInfo`, `useCancelBooking`, `useConfirmAttendance`, `useRescheduleBooking`) recuperados desde el historial de versiones de OneDrive.com (no estaban ni en la copia corrupta local). `index.css` con la paleta real (#2C3F27, #F9BFC6, #B9BD5B, #FFF9F1, #EFE7DA) + tipografía Jost/Century Gothic, logo y fuentes recuperados de la misma fuente. US-41: quitado el `CardTitle` redundante ("Cancelar cita") de la pantalla de cancelación. Verificado con Playwright (4 pantallas × desktop/375px) antes de deployar; test-harness backend 148/148 sin cambios. Confirmado en la URL pública real (`?type=initial` → 200, logo/fuentes/colores del brandbook presentes en el HTML). **Deploy activo.** |
...
34. **`appsscript.json` de la raíz sin `runtimeVersion: "V8"` tumbó el deploy de testing (18 jul) — regresión introducida en el commit `5e02188`.** Historia: `dist/appsscript.json` traía `"runtimeVersion": "V8"` desde algún momento anterior al historial de commits visible, pero el `appsscript.json` de la RAÍZ nunca lo tuvo — nadie lo notó porque `dist/` no se reconstruía desde cero en cada sesión. Al arreglar `build.sh` para que copiara el manifest de la raíz a `dist/` (commit `5e02188`, sesión anterior), esa copia SOBREESCRIBIÓ el `dist/appsscript.json` bueno con uno sin `runtimeVersion` — Apps Script cayó de vuelta al motor Rhino (deprecado, ya no soportado), y los 4 links del portal empezaron a dar "El entorno de ejecución de Rhino dejó de estar disponible". Deploy v29 salió roto; v30 lo corrigió agregando `runtimeVersion: "V8"` a la raíz. **Regla reforzada:** el `appsscript.json` de la RAÍZ del repo es ahora la ÚNICA fuente de verdad completa del manifest (incluyendo `runtimeVersion`) — `dist/appsscript.json` nunca debe editarse a mano de forma independiente de la raíz; cualquier campo que falte en la raíz se propaga al deploy real. Antes de confiar en que "arreglar el flujo de build" no tiene efectos secundarios, comparar explícitamente el archivo viejo (el que se generaba a mano) contra el nuevo (el que genera el script), campo por campo — no asumir que la fuente "correcta" (la raíz, versionada en git) ya tenía todo lo que el archivo generado a mano sí tenía.
35. **`backend/package.json` tenía su propio script `"build": "tsc"`, que NO copiaba nada a `dist/` de la raíz — gap detectado y corregido durante la reconstrucción del entorno local (27 jul).** El flujo real de deploy pasa por `build.sh` (raíz), que sí encadena `tsc` + `cp backend/dist/* dist/` + `cp backend/templates/*.html dist/` + `cp frontend/dist/* dist/` + `cp appsscript.json dist/appsscript.json`. Pero si alguien corre `npm run build` DENTRO de `backend/` directamente (o algún script/hook llama a ese script en vez de a `build.sh`), solo se ejecutaba `tsc` — el `backend/dist/app.js` quedaba actualizado, pero el `dist/app.js` de la raíz (que es el que `clasp push` sube de verdad, según `rootDir: "./dist"` en `.clasp.json`) se quedaba con la versión vieja. `clasp push`/`clasp deploy` no fallan ni avisan en este caso — suben `dist/` tal cual está, exitosamente, con código desactualizado y sin ningún error visible. **Regla reforzada:** `backend/package.json` → `"build"` ahora también copia `backend/dist/app.js` y `backend/templates/*.html` hacia `../dist/` (replicando el mismo paso que hace `build.sh`), para que ambos caminos (build desde la raíz o build desde `backend/`) dejen `dist/` consistente. Antes de confiar en un `clasp push` "exitoso", verificar que `dist/app.js` en la raíz realmente contiene el cambio esperado (ej. grep de una función nueva), no solo que el comando no tiró error. **Detalle de implementación (27 jul):** el primer intento usó `mkdir -p` + `cp` directo en el script de `"build"`, pero en este entorno Windows `npm run build` ejecuta los scripts vía `cmd.exe`, no bash — ni `mkdir -p` ni `cp` existen ahí, y el fallo es silencioso-ish (`"La sintaxis del comando no es correcta."`, sin explicar la causa real). Se resolvió moviendo la copia a un script `backend/copy-to-dist.js` (Node puro, `fs.mkdirSync`/`fs.copyFileSync`), invocado como `tsc && node copy-to-dist.js` — portable entre cmd.exe/PowerShell/bash porque no depende de comandos de shell, solo de Node.
36. **El frontend de US-31 (`manage-booking.tsx` + 4 hooks) y el brandbook de US-28 se habían perdido del disco por completo, no solo del historial de git — se recuperaron el 28 jul vía el historial de versiones del sitio web de OneDrive.com (distinto de la copia local corrupta que ya se conocía).** El incidente de OneDrive de la nota #35/commit `ff26301` (27 jul) había recuperado el **backend** (`app.ts` + plantillas de correo) desde una copia de trabajo sana en disco y `clasp pull`, pero esa recuperación nunca tocó `frontend/`. Al auditar el repo para US-41 se descubrió que `manage-booking.tsx` no existía en ningún lado accesible: ni en el working tree, ni en ningún commit de git (`git log --all --diff-filter=A` vacío), ni en el VS Code Local History, ni siquiera en la copia corrupta de OneDrive en disco (que sí conservaba `components/ui/` y las fuentes `.ttf`, pero no un solo archivo `.tsx` de la app). La única fuente que sí lo tenía era el historial de versiones web de OneDrive.com (accesible solo desde el navegador, no desde el archivo sincronizado en disco) — el usuario restauró una versión anterior ahí y descargó el `frontend/src/` completo como zip. Comparado archivo por archivo contra el repo actual antes de fusionar: todo el diff resultó aditivo (nunca hubo que elegir entre versiones en conflicto). Recuperado también el brandbook de US-28 (`index.css`, logo, fuentes Jost) que venía en la misma versión restaurada. **Regla reforzada:** cuando una recuperación de incidente (OneDrive, corrupción de git, etc.) toca solo una parte del stack (ej. backend), no asumir que el resto (frontend) está a salvo solo porque no se mencionó — auditar cada capa por separado, incluyendo fuentes de recuperación no obvias (historial web de un servicio de sync, no solo su carpeta local sincronizada).
