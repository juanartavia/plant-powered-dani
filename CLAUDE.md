...
> Última actualización: 27 julio 2026 (noche) — **US-33 (RF-2.5) cerrada, aprobada visualmente por el usuario:** la alerta interna de cancelación tardía dejó de armar su HTML a mano en TypeScript y ahora usa una plantilla real con el branding del sistema (`backend/templates/notificacion_cancelacion_tardia.html`, basada en el esqueleto de `notificacion_interna_nueva_cita.html`), diferenciada con badge/callout/CTA en rojo (#C0392B). Ver detalle completo en sección 3. Test-harness 148/148 — trade-off documentado: el mock de `createTemplateFromFile()` ya no permite verificar contenido literal del HTML, esa verificación queda en revisión visual manual. Deploy activo: v46. **Nota:** las secciones 0/11/12 de este documento reflejan trabajo hasta v27 con precisión; el historial entre v28 y v45 (US-13/US-30, US-31, US-32, primera versión de US-33, US-28) sigue pendiente de reconstruir (ver Sprint 3, sección 11) — no bloquea nada, pero léase con esa salvedad.
...
| Versión actual | **v46** (deployment actualizado 27 jul 2026, US-33: plantilla de branding real para la alerta de cancelación tardía) |
| Repo | https://github.com/juanartavia/plant-powered-dani |
...
| Harness de pruebas backend | `backend/test-harness/` — 148 aserciones, todas pasando (27 jul 2026) |
...
| v46 | 27 jul (noche) | **US-33 — plantilla de branding real para la alerta de cancelación tardía**, aprobada visualmente por el usuario (capturas de Gmail real, nutrición y pilates). `renderNotificacionCancelacionTardia()` pasó de HTML armado a mano en TypeScript a `HtmlService.createTemplateFromFile("notificacion_cancelacion_tardia")`, mismo patrón que `renderNotificacionInterna()`. Detalle completo en sección 3. **Deploy activo.** |
