...
> Última actualización: 18 julio 2026 — **Fix confirmado en Gmail real:** el bloque de dirección del correo de confirmación (nutrición, presencial) mostraba el `<br>` como texto literal en vez de saltos de línea — causa: `<?= direccion ?>` escapa HTML por default en HtmlService. Cambiado a `<?!= direccion ?>` en ambas plantillas de nutrición (ES/EN). Invitación nativa de Google Calendar suprimida (`sendUpdates: 'none'`). Scope `userinfo.email` agregado al manifest. US-11 (correo de confirmación) sigue en progreso — render verificado, envío real pendiente de confirmar (recordar aceptar el nuevo permiso de Google al correr `testSendConfirmationEmails()` desde el editor). Deploy activo: v27.
